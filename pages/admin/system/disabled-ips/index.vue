    ...
    <div class="mb-6 mb-md-12 d-flex align-center">
      <div class="text-black">
        <h3 class="text-body-1 font-weight-bold">
          {{ $t('page.disabledIps.title') }}
        </h3>
        <p class="text-body-2 text-medium-emphasis">
          {{ $t('page.disabledIps.subtitle') }}
        </p>
      </div>
      <v-spacer />
      <v-btn
        to="/admin/system/disabled-ips/add"
        color="primary"
        class="text-body-1 d-none d-sm-flex"
        rounded="pill"
        variant="flat"
      >
        <Icon name="plus" size="22px" /> {{ $t('label.addIp') }}
      </v-btn>
      <v-btn
        class="d-sm-none"
        to="/admin/system/disabled-ips/add"
      ...
      >
        ...
        <template #item.action="{ item }">
          <AdminTableActionMenu
            :editPath="`/admin/system/disabled-ips/edit/${item.id}`"
          >
...
const {
  ...
  data: apiData,
  refresh: refreshData,
} = await useFetch('https://dummyjson.com/c/d2eb-06d1-4834-b095', {
  lazy: true,
})
...
useHead({
  title: t('page.disabledIps.head.title'),
