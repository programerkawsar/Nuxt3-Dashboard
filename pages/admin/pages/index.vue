    ...
    <div class="mb-6 mb-md-12 d-flex align-center">
      <div class="text-black">
        <h3 class="text-body-1 font-weight-bold">
          {{ $t('page.pageManagement.title') }}
        </h3>
        <p class="text-body-2 text-medium-emphasis">
          {{ $t('page.pageManagement.subtitle') }}
        </p>
      </div>
      <v-spacer />
      <v-btn
        ...
        variant="flat"
      >
        <Icon name="plus" size="22px" /> {{ $t('label.addPage') }}
      </v-btn>
      <v-btn
        class="d-sm-none"
        to="/admin/pages/add"
    ...
    <v-card variant="text" rounded="xl" border>
      ...
      >
        ...
        <template #top>
          <AdminTableHeader
            active="search"
            :filters="['timeRange', 'search', 'status']"
            :export="false"
          >
            <template #refreshBtn>
              <v-btn
        ...
        <template #item.action="{ item }">
          <AdminTableActionMenu :editPath="`/admin/pages/edit/${item.id}`">
            <template #view>
...
const {
  ...
  data: apiData,
  refresh: refreshData,
} = await useFetch('https://dummyjson.com/c/75e3-aa2d-4517-ae32', {
  lazy: true,
})
...
useHead({
  title: t('page.pageManagement.head.title'),
