    ...
    <div class="mb-6 mb-md-12 d-flex align-center">
      <div class="text-black">
        <h3 class="text-body-1 font-weight-bold">
          {{ $t('page.financePlans.title') }}
        </h3>
        <p class="text-body-2 text-medium-emphasis">
          {{ $t('page.financePlans.subtitle') }}
        </p>
      </div>
      <v-spacer />
      <v-btn
        to="/admin/finance/plans/add"
        color="primary"
        class="text-body-1 d-none d-sm-flex"
        rounded="pill"
        variant="flat"
      >
        <Icon name="plus" size="22px" /> {{ $t('label.addPlan') }}
      </v-btn>
      <v-btn
        class="d-sm-none"
        to="/admin/finance/plans/add"
    ...
    <v-card variant="text" rounded="xl" border>
      ...
      >
        ...
        <template #top>
          <AdminTableHeader
            active="status"
            :filters="['status', 'search', 'timeRange']"
            :export="false"
          >
            <template #refreshBtn>
              <v-btn
        ...
        <template #item.active="{ item }">
          <v-chip :color="item.active ? 'success' : 'error'">
            <span class="text-capitalize text-body-2">
              {{ $t(`label.${item.active ? 'active' : 'inactive'}`) }}
            </span>
          </v-chip>
        </template>
        ...
        <template #item.action="{ item }">
          <AdminTableActionMenu
            :editPath="`/admin/users/roles/edit/${item.id}`"
          >
            <template #view>
              <AdminDialogsViewTableData show="userPermission" />
            </template>
...
const {
  ...
  data: apiData,
  refresh: refreshData,
} = await useFetch('https://dummyjson.com/c/b658-bf70-4205-a620', {
  lazy: true,
})
...
useHead({
  title: t('page.financePlans.head.title'),
