<template>
  <v-sheet color="transparent" class="mx-auto" :max-width="1000">
    <div class="mb-6 mb-md-12 d-flex align-center">
      ...
      <v-spacer />
      <v-btn
        to="/admin/users/roles/add"
        color="primary"
        class="text-body-1 d-none d-sm-flex"
        rounded="pill"
        variant="flat"
      >
        <Icon name="plus" size="22px" /> {{ $t('label.addRole') }}
      </v-btn>
      <v-btn
        class="d-sm-none"
        to="/admin/users/roles/add"
      ...
      >
        ...
        <template #item.permissions="{ item }">
          <p class="text-black text-body-2">
            {{ abbreviateNumber(item.permissions) }}
          </p>
        </template>
...
const {
  ...
  data: apiData,
  refresh: refreshData,
} = await useFetch('https://dummyjson.com/c/a36f-cee4-4274-beed', {
  lazy: true,
})
